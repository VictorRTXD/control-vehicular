...
last_detection = {'plate': None, 'timestamp': None, 'confidence': 0}
detection_cooldown = 3  # segundos entre detecciones de la misma placa
...
def generate_frames():
    ...
    global last_detection, pending_validations
    ...
    while camera_active and camera:
        ...
        # Procesar detección cada cierto tiempo
        time_since_last = (current_time - datetime.fromisoformat(last_detection['timestamp'])).total_seconds() if last_detection['timestamp'] else 999
        ...
        if time_since_last > detection_cooldown:
            ...
            try:
                ...
                if detected_plate and detected_plate != last_detection['plate']:
                    last_detection['plate'] = detected_plate
                    last_detection['timestamp'] = current_time.isoformat()
        ...
        if last_detection['plate']:
            status_text = f"Ultima placa: {last_detection['plate']}"
            known_status = " [CONOCIDO]" if last_detection['plate'] in known_vehicles else " [DESCONOCIDO]"
